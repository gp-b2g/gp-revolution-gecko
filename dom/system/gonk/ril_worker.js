...
let RIL = {
  ...
  setIntelNwWhenRilOn: function setIntelNwWhenRilOn() {
	...
	this.getSignalStrength();
	debug("RIL.iccInfo.iccid!!!!!" + RIL.iccInfo.iccid);
  ...
  setRoamingPreference: function setRoamingPreference(options) {
	...
	debug("DENTRO DE SetRoamingPreference, options: " + options);
    if (roamingMode === -1) {
	  debug("romaingMode es -1!!!! options: " + options);
  ...
  setVoicePrivacyMode: function setVoicePrivacyMode(options) {
	debug("DENTRO DE setVoicePrivacyMode, options: " + options);
...
RIL[REQUEST_GET_IMSI] = function REQUEST_GET_IMSI(length, options) {
  debug("dentro getIMSI2!!!!");
  if (options.rilRequestError) {
	debug("casque en getISMI2");
    return;
  }
...
let ICCUtilsHelper = {
  ...
  parseMccMncFromImsi: function parseMccMncFromImsi(imsi, mncLength) {
	debug("parseando mcc y mnc!!!!"); 
	if (!imsi) {
	  debug("cagada, no tiene IMSI!!!!!!!");
      return null;
    }
